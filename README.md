...
5) **Walk, trail, path, cycle (circuit), and Eulerian trail/circuit:** The concepts that define the sequences of nodes and edges within a graph are reviwed here. The conditions for a graph to have an Eulerian trail or circuit (cycle) are also mentioned.
